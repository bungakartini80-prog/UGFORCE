...
<style>
    ...
    *, *::before, *::after { box-sizing: border-box; } body { margin: 0; padding: 0; }
    ...
    .btn-glow { background: linear-gradient(135deg, #f59e0b, #ea580c); color: #ffffff !important; border: none; border-radius: 14px; padding: 16px; font-size: 15px; font-weight: 800; cursor: pointer; display: flex; justify-content: center; align-items: center; gap: 10px; width: 100%; transition: all 0.4s; text-transform: uppercase; font-family: var(--font-mono); box-shadow: 0 10px 20px rgba(245, 158, 11, 0.2);}
    ...
    <main class="cms-main">
        ...
        <div class="glass-card form-panel animate-float d-2">
            <div style="font-size: 24px; font-weight: 800; margin-bottom: 30px; display: flex; align-items: center; gap: 12px;">
                <div style="width: 45px; height: 45px; background: rgba(245, 158, 11, 0.1); color: var(--warning); border-radius: 12px; display: flex; align-items: center; justify-content: center;"><i class="fas fa-user-edit"></i></div>
            ...
            <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
                ...
                <div class="form-group" style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                    <div>
                        <label class="form-label">Password Baru <span style="text-transform:none; font-weight:500; color:var(--text-tertiary);">(Opsional)</span></label>
                        <input type="password" name="password" class="form-control" placeholder="Kosongkan jika tidak mau diganti">
                    </div>
                    <div>
                        <label class="form-label">Hak Akses (Role)</label>
                        <select name="role" class="form-control" required>
                            <option value="student" {{ $user->role == 'student' ? 'selected' : '' }}>Mahasiswa (Student)</option>
                ...
                <button type="submit" class="btn-glow mt-4">
                    <i class="fas fa-sync-alt"></i> Simpan Perubahan Data User
                </button>
            </form>
        </div>
    </main>
