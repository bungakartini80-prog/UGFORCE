...
<style>
    ...
    .table-toolbar { padding: 20px 24px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px;}
    ...
    .btn-warning-outline { background: rgba(245, 158, 11, 0.1); color: var(--warning); border: 1px solid rgba(245, 158, 11, 0.3); }
    .btn-warning-outline:hover { background: var(--warning); color: #fff; }
        ...
        <div class="glass-card table-panel animate-float d-2">
            <div class="table-toolbar">
                <div class="panel-header">
                    <i class="fas fa-calendar-alt text-[#3b82f6]"></i> Daftar Jadwal Kuliah Resmi
                </div>
                <div class="flex items-center gap-3">
                    <form method="POST" action="{{ route('admin.schedules.reset-all') }}" class="m-0" onsubmit="return confirm('Reset semua status kelas selesai ke Ready dan kosongkan ruangan kelas?');">
                        ...
                        <button type="submit" class="btn-glow" style="background: linear-gradient(135deg, #ef4444, #b91c1c); box-shadow: 0 5px 15px rgba(239, 68, 68, 0.3);">
                            <i class="fas fa-undo"></i> Reset Semua Kelas Selesai ({{ $schedules->whereIn('status', ['selesai', 'selesai_selesai'])->count() }})
                        </button>
                    </form>
            ...
            @if($schedules->isEmpty())
                <div class="text-center py-16">
                    <div class="w-20 h-20 rounded-full bg-slate-100 dark:bg-white/5 flex items-center justify-center mx-auto mb-4 text-slate-400">
                        <i class="fas fa-calendar-times text-3xl opacity-50"></i>
                    </div>
                                ...
                                <tr>
                                    <td>
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <span class="td-primary block">{{ $schedule->lecturer->name }}</span>
                                                <span class="text-xs text-slate-500 dark:text-slate-400 font-mono">{{ $schedule->lecturer->email }}</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <span class="font-bold text-slate-800 dark:text-white block">{{ $schedule->subject }}</span>
                                        <span class="badge-cyber badge-success mt-1">Kelas: {{ $schedule->class_name }}</span>
                                    </td>
                                    <td>
                                        <span class="badge-cyber badge-warning mb-1">{{ $schedule->day_of_week }}</span>
                                    ...
                                    <td class="text-center">
                                        <span class="badge-cyber badge-warning" style="font-size: 13px;">
                                            {{ $schedule->room->name }}
                                        </span>
                                    </td>
                                    <td class="text-center">
                                        ...
                                        @elseif($schedule->status === 'selesai_selesai')
                                            <span class="badge-cyber border-slate-500 text-slate-400">
                                                <i class="fas fa-check-double text-sm"></i> Kelas Selesai
                                            </span>
                                        @else
                                            <span class="badge-cyber badge-warning">
                                        ...
                                        <div style="display: flex; gap: 8px; justify-content: center;">
                                            @if($schedule->status === 'selesai' || $schedule->status === 'selesai_selesai')
                                                <form method="POST" action="{{ route('admin.schedules.reset', $schedule) }}" class="m-0" onsubmit="return confirm('Reset status jadwal kuliah ini menjadi Ready agar Dosen dapat konfirmasi kembali?');">
                                                    @csrf
                                                    <button type="submit" class="btn-action btn-warning-outline" title="Reset Status ke Ready">
                                            ...
                                            <form method="POST" action="{{ route('admin.schedules.destroy', $schedule) }}" class="m-0" onsubmit="return confirm('Apakah Anda yakin ingin menghapus jadwal kuliah ini?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn-action btn-danger-outline" title="Hapus Jadwal">
